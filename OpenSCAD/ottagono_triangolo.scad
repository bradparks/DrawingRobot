...
translate([xBoard+7.8/2,0,0]) {
	...
	translate([-60,-60,-.05]) cube([60,60*2,h+.1]);
}
				for(r=[-4:1:1]) rotate([0,0,360/8*r+67.5])
			translate([0,-larghezzaBarre/2,0]) cube([59,larghezzaBarre,h]);
	}
difference(){
    union() {
       // Se cuore = 1
